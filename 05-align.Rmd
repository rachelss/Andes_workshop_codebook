...
```
bowtie2 -p 20 -x /data3/schwartzlab/rachel/human_teaching/ref/hg38_Ensembl -1 /data3/schwartzlab/human_CEU_trio/NA12878/U0a_CGATGT_L001_R1_001.fastq.gz -2 /data3/schwartzlab/human_CEU_trio/NA12878/U0a_CGATGT_L001_R2_001.fastq.gz -S results/U0a_CGATGT_L001_001.sam
```
...
That's going to be in `/data3/schwartzlab/rachel/human_teaching/ref/` and its name is hg38.
...
We'll also have the computer send you an email when the job is done (enter your email address - don't just copy-paste).
...
Use `ls` to get the name of your output file (everyone's is unique, but it will start with slurm and end with .out).
Then use `cat` (command) and the name of the file (argument) to see its contents.
...
`grep -v '^@SQ' results/U0a_CGATGT_L001_001.sam`
...
`grep -v '^@SQ' results/U0a_CGATGT_L001_001.sam  |less`
...
As before, you need to load a module to access the samtools program.
Replace your bowtie module with `SAMtools/1.10-GCC-8.3.0`.
