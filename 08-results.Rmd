...
myseqs <- read.dna("concatenated.fasta",format="fasta",as.matrix=FALSE)
```
...
```
write.nexus.data(as.character(myseqs),"concatenated.nex",interleaved=FALSE,gap="-",missing="N")
myseqs.nex <- readLines("concatenated.nex")
```
...
write(myseqs.nexus.withmb,file="concatenated.nex.mb")
```

Now run (on the command line)
```
../../shared/bin/mb "concatenated.nex.mb"
```
